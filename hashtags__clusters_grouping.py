...
def main():	
    # load datasets
    df = pd.read_csv("OutputStreaming6.csv")
    ...
    df['Clusters'] = False
    for index in range(0, df['Hashtag'].count()):
        df['Clusters'].iloc[index] = clusters[index]
    #df.to_csv('results_1.csv', sep='\t', encoding='utf-8', index=False)
 
    # grouping users by cluster values and hashtags
    grouped_by = df.groupby(['Clusters', 'Hashtag'])
    df = pd.DataFrame(columns= df.columns)
    for cluster, data in grouped_by:
        df = df.append(data)
    df.apply(lambda x: x).to_csv('grouped_by_clusters.csv', sep='\t', encoding='utf-8', index=False)
